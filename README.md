...
## Compiled UI Look & Feel

Below is a production-style compiled UI preview that reflects the current premium dashboard direction.

![Compiled UI preview](docs/images/compiled-ui-preview.svg)

> For an exact runtime screenshot from your environment, run the production app (`npm run build && npm run start`) and capture from browser.
